...
import IndustrySelectModal from "../../buttonFlow/IndustrySelector"
import StepSelector from "../../buttonFlow/StepSelector"
import NearMissImporter from "../../buttonFlow/NearMissImporter"
import CauseImporter from "../../buttonFlow/CauseImporter"

type EditableRow = { id: number, work: string, hazard: string, law: string, action: string, proof?: File | null }

const columns: Column[] = [
{ key: "work", label: "공정(작업)", minWidth: 90, align: "left", titleKey: "work" },
...
if (key === "work" && typeof val === "string") return { ...row, work: val, law: "" }
return { ...row, [key]: val }
}))
}
...
<div className="mt-1 flex justify-start"><Button variant="rowAdd" onClick={addRow} className="flex items-center gap-1 text-sm">+ 직접입력</Button></div>
</div>
</div>
<div className="mt-5 flex justify-end">
<Button variant="step" onClick={() => navigate("/risk-assessment/methods/frequency/step2")} className="py-3">저장 후 다음으로<ChevronRight size={18} className="ml-2" /></Button>
</div>

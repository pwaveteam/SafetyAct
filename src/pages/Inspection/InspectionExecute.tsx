...
const setConfirmed=useCallback((id:number,checked:boolean)=>{setRows(p=>p.map(r=>r.id===id?{...r,confirmed:checked}:r))},[])
const setNote=useCallback((id:number,val:string)=>{setRows(p=>p.map(r=>r.id===id?{...r,note:val}:r))},[])
const allDone=rows.length>0&&rows.every(r=>r.confirmed)

const columns:Column[]=[
{key:"__no",label:"번호",minWidth:60},
{key:"content",label:"점검세부내용",minWidth:520,renderCell:(row)=>(<div className="text-left whitespace-pre-wrap break-words">{(row as ExecItemRow).content}</div>)},
{key:"confirmed",label:"점검결과",minWidth:80,renderCell:(row)=>{const r=row as ExecItemRow;return(<div className="w-full flex justify-center"><label className="inline-flex items-center gap-1 select-none"><span>확인</span><Checkbox checked={r.confirmed} onChange={()=>setConfirmed(r.id,!r.confirmed)}/></label></div>)}},
{key:"note",label:"조치/비고",minWidth:320,renderCell:(row)=>(<EditableCell value={(row as ExecItemRow).note} onChange={(val:string)=>setNote((row as ExecItemRow).id,val)} placeholder="조치사항 또는 비고 입력" maxLength={200}/>)}
]

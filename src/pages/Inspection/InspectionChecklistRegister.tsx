...
type ItemRow=DataRow&{id:number;content:string;isEditing?:boolean;draft?:string}
...
const makeColumns=(renderContent:(row:DataRow)=>React.ReactNode):Column[]=>[
{key:"__no",label:"번호",minWidth:50},
{key:"content",label:"점검세부내용",minWidth:500,renderCell:renderContent}
]
...
const tableData=useMemo<DataRow[]>(()=>rows.map((r,idx)=>({...r,__no:idx+1})),[rows])
...
()=>makeColumns((row:DataRow)=>{
const r=row as ItemRow
if(r.isEditing){
const isNew=(r.content==="")
return(
<div className="flex items-center gap-2 min-h-[40px]" data-row-id={r.id}>
...
<input data-edit="true" className={`${INPUT_CLASS} flex-1 h-[32px]`} placeholder="점검 세부내용을 입력하세요" value={r.draft??""} onChange={e=>changeDraft(r.id,e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){commitEdit(r.id)}if(e.key==="Escape"){cancelEdit(r.id,isNew)}}}/>
<button type="button" aria-label="저장" className="inline-flex items-center justify-center p-1 rounded-md border border-[#D1D5DB] hover:bg-gray-50" onClick={()=>commitEdit(r.id)} title="저장"><Check size={16}/></button>
<button type="button" aria-label="취소" className="inline-flex items-center justify-center p-1 rounded-md border border-[#D1D5DB] hover:bg-gray-50" onClick={()=>cancelEdit(r.id,isNew)} title="취소"><X size={16}/></button>
</div>
)
}
return(
<button type="button" className="w-full text-left" onClick={()=>startEdit(r.id)} title="클릭하여 편집">
<div className="flex items-center gap-2 min-h-[40px]">
<span className="drag-handle inline-flex items-center justify-center cursor-grab text-[#9AA4B2] hover:text-[#6B7485] active:cursor-grabbing select-none" title="드래그하여 순서 변경" aria-label="드래그 핸들"><GripVertical size={16}/></span>
<span className="text-left">{r.content}</span>
</div>
</button>
)
}),
...
useEffect(()=>{const tbody=tableWrapRef.current?.querySelector("tbody");if(!tbody)return;const sortable=Sortable.create(tbody as HTMLElement,{animation:150,handle:".drag-handle",ghostClass:"opacity-60",onEnd:evt=>{const oldIndex=evt.oldIndex??-1;const newIndex=evt.newIndex??-1;if(oldIndex<0||newIndex<0||oldIndex===newIndex)return;setRows(prev=>{const next=[...prev];const[moved]=next.splice(oldIndex,1);next.splice(newIndex,0,moved);return next})}});return()=>sortable.destroy()},[rows.length])
...
<DataTable columns={columns} data={tableData} onCheckedChange={setCheckedIds}/>

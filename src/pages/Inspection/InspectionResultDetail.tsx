...
type DetailItemRow=DataRow&{id:number;content:string;result:"OK"|"NG";note:string}
...
{id:1,content:"분전반 외관 이상 유무 확인",result:"OK",note:"양호"},
{id:2,content:"누전차단기 시험버튼 작동 확인",result:"NG",note:"교체 필요"},
{id:3,content:"접지상태(접지선 손상/이탈) 점검",result:"OK",note:"정상"},
{id:4,content:"과열·변색 흔적 여부 확인",result:"NG",note:"점검 시 변색 발견"},
{id:5,content:"작업장 내 임시배선 정리상태 점검",result:"OK",note:"정리 완료"}
])

const columns:Column[]=[
{key:"__no",label:"번호",minWidth:60},
{key:"content",label:"점검세부내용",minWidth:520,renderCell:(row)=>(<div className="text-left whitespace-pre-wrap break-words">{(row as DetailItemRow).content}</div>)},
{key:"result",label:"점검결과",minWidth:80,renderCell:(row)=>{const r=row as DetailItemRow;const checked=r.result==="OK";return(<div className="w-full flex justify-center"><label className="inline-flex items-center gap-1 select-none pointer-events-none"><span>확인</span><Checkbox checked={checked} onChange={()=>{}}/></label></div>)}},
{key:"note",label:"조치/비고",minWidth:320,renderCell:(row)=>(<div className={`${BOX} w-full bg-white`}>{(row as DetailItemRow).note||"-"}</div>)}
]

...
if (!selectedId) { alert("점검표를 선택하세요"); return }
const selected = MOCK_CHECKLISTS.find(chk => chk.id === selectedId)
if (!selected) { alert("점검표를 찾을 수 없습니다"); return }
onConfirm(selected.name)
onClose()
}, [onClose, onConfirm, selectedId])

return (
<Dialog title="점검표 선택" onClose={onClose} size="lg" className="w-[96vw] !max-w-[600px]">
...
<thead>
<tr>
<th className={TH}>점검표명</th>
</tr>
</thead>
<tbody>
{MOCK_CHECKLISTS.map(chk => {
const active = chk.id === selectedId
return (
<tr
key={chk.id}
onClick={() => selectRow(chk.id)}
className={`cursor-pointer select-none ${active ? `bg-[${COLOR.rowHover}]` : `hover:bg-[${COLOR.rowHover}]`}`}
>
<td className={`${TD} truncate`}>{chk.name}</td>
</tr>
)
})}
{MOCK_CHECKLISTS.length === 0 && (
<tr><td className={TD}>등록된 항목이 없습니다.</td></tr>
)}
</tbody>
</table>
</div>
...
<Button variant="primary" onClick={confirm} className="min-w-[90px]">확인</Button>
</div>
</Dialog>
)
}

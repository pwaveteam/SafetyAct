...
import { Upload, Download, Printer, Trash2, Save } from "lucide-react"
...
const PolicyGoal:React.FC=()=>{const[values,setValues]=useState<{[key:string]:string}>({
year:"2025",
...
managerSignature:""});const fileInputRef=useRef<HTMLInputElement|null>(null)
...
const handleFileChange=(e:React.ChangeEvent<HTMLInputElement>)=>{if(e.target.files&&e.target.files.length>0){setValues(prev=>({...prev,managerSignature:e.target.files![0].name}));alert(`파일 선택됨: ${e.target.files[0].name}`)}}
...
{label:"경영방침 업로드",name:"managerSignature",type:"custom",customRender:(<><Button variant="action" onClick={openFileDialog} style={{minWidth:120}} className="flex items-center gap-1"><Upload size={18}/>경영방침 업로드</Button><input type="file" ref={fileInputRef} className="hidden" onChange={handleFileChange} accept=".hwp,.doc,.docx,.pdf"/></>)}
]
return (

...
export type ReceivedDetail={date:string;type:string;content:string;drafter:string;status:"결재완료"|"결재대기"}
export type SentDetail={date:string;document:string;status:"진행"|"완료"|"대기";progress:string;finalApprover:string}
type Props={variant:"received";row:ReceivedDetail;onClose:()=>void}|{variant:"sent";row:SentDetail;onClose:()=>void}

const renderStatus=(variant:"received"|"sent",status:string)=>{if(variant==="received"){if(status==="결재완료")return<Badge color="gray">결재완료</Badge>;return<Badge color="orange">결재대기</Badge>}if(status==="진행")return<Badge color="green">진행</Badge>;if(status==="완료")return<Badge color="gray">완료</Badge>;return<Badge color="orange">대기</Badge>}
...
return(<Dialog title="결재 상세 정보" onClose={onClose} size="md">
{variant==="received"?(<div className="grid grid-cols-3 gap-y-3 text-sm py-2">
...
<div className="font-medium text-gray-600">상태</div><div className="col-span-2 flex items-center gap-2">{renderStatus("received",row.status)}{isPending&&<Button variant="signature"/>}{isSigned&&<Button variant="signed"/>}</div>
</div>):(<div className="grid grid-cols-3 gap-y-3 text-sm py-2">
<div className="font-medium text-gray-600">기안일</div><div className="col-span-2">{row.date}</div>

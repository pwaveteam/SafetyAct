...
const PX_TO_PT=0.75
const HEADER_PT=20*PX_TO_PT
const META_PT=11*PX_TO_PT
const BODY_PT=13*PX_TO_PT
const COLOR_BLACK:[number,number,number]=[0,0,0]
const COLOR_CHARCOAL:[number,number,number]=[51,51,51]
const LINE=6

const drawHeader=(doc:jsPDF,category:string,metaRight?:{id?:string;no?:string}):number=>{
doc.setFont("NotoSansKR","bold");doc.setFontSize(HEADER_PT);doc.setTextColor(...COLOR_BLACK);const yTitle=mm(20);const xLeft=mm(20);doc.text(category,xLeft,yTitle)
const rightX=doc.internal.pageSize.getWidth()-mm(20)
let yMeta=yTitle
...
const addSectionTitle=(doc:jsPDF,text:string,y:number):void=>{
doc.setFont("NotoSansKR","bold");doc.setFontSize(BODY_PT);doc.setTextColor(...COLOR_BLACK);doc.text(text,mm(20),y);doc.setFont("NotoSansKR","normal")
}
...
addSectionTitle(doc,label,startY)
const y=startY+LINE-1
doc.setFont("NotoSansKR","normal");doc.setFontSize(BODY_PT);doc.setTextColor(...COLOR_CHARCOAL)
...
startY:y,
styles:{font:"NotoSansKR",fontSize:BODY_PT as number,cellPadding:3,lineWidth:0.1,textColor:COLOR_CHARCOAL},
headStyles:{fillColor:[240,240,240],textColor:COLOR_BLACK,font:"NotoSansKR",fontStyle:"bold",fontSize:BODY_PT as number},
bodyStyles:{textColor:COLOR_CHARCOAL,fontSize:BODY_PT as number},
margin:{left:mm(20),right:mm(20)},
tableLineColor:[200,200,200],
tableLineWidth:0.1,
didDrawPage:data=>{if(data.pageNumber>1){doc.setFontSize(BODY_PT);doc.setTextColor(...COLOR_BLACK);doc.text(String(data.pageNumber),doc.internal.pageSize.getWidth()-mm(20),mm(15),{align:"right"})}}
}
autoTable(doc,{head:[head],body,...options})
const pluginDoc=doc as JsPdfWithAutoTable
...
let y=startY+LINE*2
const x=doc.internal.pageSize.getWidth()-mm(20)-35
...
const addAttachments=(doc:jsPDF,files:RFAttachment[],startY:number):number=>{
addSectionTitle(doc,"첨부목록",startY)
return addKeyValues(doc,files.map(f=>({label:"첨부",value:f.name})),startY+LINE)
}
...
register,has,
async generate<I>(key:string,input:I):Promise<void>{
if(!registry.has(key))throw new Error(`등록되지 않은 리포트 포맷: ${key}`)
await ensureFonts()
const tpl=registry.get(key) as RFTemplate<I>
const spec=await tpl.map(input)
const docIdKV=spec.keyValues.find(k=>k.label==="문서ID")
const docNoKV=spec.keyValues.find(k=>k.label==="문서번호")
const filteredKVs=spec.keyValues.filter(k=>k.label!=="문서ID"&&k.label!=="문서번호")
const doc=new jsPDF({unit:"mm",format:"a4"})
applyFonts(doc)
const yHr=drawHeader(doc,spec.title,{id:docIdKV?.value,no:docNoKV?.value})
let y=yHr+LINE
y=addKeyValues(doc,filteredKVs,y)
for(const s of spec.sections)y=addParagraph(doc,s.title,s.text,y+LINE-1)
...
const name=spec.fileName??`Report_${new Date().toISOString().slice(0,10).replace(/-/g,"")}.pdf`
doc.save(name)
}
}
...
export type RFTbmInput={id:number|string;factory:string;tbmName:string;location:string;date:string;startTime:string;endTime:string;durationText:string;targetText:string;participantsText:string;leader:string;content:string;remark:string;riskProcessTitle?:string;riskItems:{hazard:string;countermeasure:string}[];attendees:RFAttendee[];qrDataUrl?:string;attachments?:RFAttachment[]}
...
{label:"일시",value:`${new Intl.DateTimeFormat("ko-KR",{dateStyle:"long"}).format(new Date(v.date))} ${v.startTime}~${v.endTime} (${v.durationText})`},
